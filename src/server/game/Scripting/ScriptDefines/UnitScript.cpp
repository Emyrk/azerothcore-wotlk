...
{
    CALL_ENABLED_HOOKS(UnitScript, UNITHOOK_ON_DEAL_MELEE_DAMAGE, script->OnDealMeleeDamage(calcDamageInfo, damageInfo, overkill));
}

void ScriptMgr::OnSendSpellNonMeleeDamageLog(SpellNonMeleeDamage* log)
{
    CALL_ENABLED_HOOKS(UnitScript, UNITHOOK_ON_SEND_SPELL_NON_MELEE_DAMAGE_LOG, script->OnSendSpellNonMeleeDamageLog(log));
}
...
{
    CALL_ENABLED_HOOKS(UnitScript, UNITHOOK_ON_SEND_ENERGIZE_SPELL_LOG, script->OnSendEnergizeSpellLog(attacker, victim, spellID, damage, powerType));
}
